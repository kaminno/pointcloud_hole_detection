...
double compute_prob(Point A_p, Point c){
    Point A_boundary;
    Point A_interior;
    ...
    A_interior.y = 1/2;
    A_interior.z = 0;
    A_corner_noise.x = 1/3;
    A_corner_noise.y = 1/3;
    A_corner_noise.z = 1/3;
    A_line.x = 1;
    A_line.y = 0;
    A_line.z = 0;
    // A_line.x = 1/2;
...
std::vector<double> some_func(PointCloud cloud, std::map<unsigned long int, std::vector<int>> neighbours, std::vector<Point> mis){
    std::cout << "---" << std::endl;
...
std::vector<double> shape_criterion(PointCloud cloud, unsigned int K, double epsilon){
